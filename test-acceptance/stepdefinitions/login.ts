...
function getJsonDatabase() {
    let jsonDatabase;
    fs.readFile('../../web-app/db.json', function (err: any, data: any) {
        if (err) {
            console.error(err)
            return err
        }
...
defineSupportCode(function ({ Given, When, Then }) {
    Given(/^o usuário "([^\"]*)" de senha "([^\"]*)" está corretamente registrado no sistema com permissões de "([^\"]*)"$/, async (username, password, permission) => {
        const db = getJsonDatabase();
        const role = permission == "user" ? "" : "admin";

        expect(
            db.user.find((user: { name: string, password: string, role: string}) => user.name == username && user.password == password && user.role == role)
        ).to.not.be.null;
    })
